...
export class HistorialDetailComponent implements OnInit {
  ...
  cargarHistorial(): void {
    ...
    this.historialService.getById(id).subscribe({
      ...
      error: (error) => {
        ...
        console.error(
          "Error al cargar el detalle del historial:",
          error
        );
  ...
  volver(): void {
    this.router.navigate([
      "/Incidentes/incidentes/historial"
    ]);
  }
  ...
  getEstadoAnterior(): string {
    ...
    if (!this.historial?.estado_anterior) {
      return "Sin estado anterior";
    }

    return this.historial.estado_anterior;
  }

  getComentario(): string {

    if (!this.historial?.comentario) {
      return "Sin comentario";
    }

    return this.historial.comentario;
  }
