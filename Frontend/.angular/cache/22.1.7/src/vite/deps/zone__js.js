...
function initZone() {
	...
	function scheduleMicroTask(task) {
		if (_numberOfNestedTaskFrames === 0 && _microTaskQueue.length === 0) nativeScheduleMicroTask(drainMicroTaskQueue);
...
function patchEventTarget(_global, api, apis, patchOptions) {
	...
	function patchEventTargetMethods(obj, patchOptions) {
		...
		if (patchOptions && patchOptions.useG !== void 0) useGlobalCallback = patchOptions.useG;
		const validateHandler = patchOptions && patchOptions.vh;
		let checkDuplicate = true;
		if (patchOptions && patchOptions.chkDup !== void 0) checkDuplicate = patchOptions.chkDup;
		let returnTarget = false;
		if (patchOptions && patchOptions.rt !== void 0) returnTarget = patchOptions.rt;
		let proto = obj;
		...
		*/
		function buildEventListenerOptions(options, passive) {
			if (!passiveSupported && typeof options === "object" && options) return !!options.capture;
		...
		*/
		...
		const makeAddListener = function(nativeListener, addSource, customScheduleFn, customCancelFn, returnTarget = false, prepend = false) {
			return function() {
				...
				if (!(!passiveSupported && typeof task.options === "boolean")) task.options = options;
				task.target = target;
				task.capture = capture;
				task.eventName = eventName;
				if (isHandleEvent) task.originalDelegate = delegate;
...
function patchTimer(window, setName, cancelName, nameSuffix) {
	...
	setNative = patchMethod(window, setName, (delegate) => function(self, args) {
			...
			args[0] = function timer() {
				try {
					...
					if (!isPeriodic && !isRefreshable) {
						if (handleId) delete tasksByHandleId[handleId];
						else if (handle) handle[taskSymbol] = null;
					}
				}
			};
		...
		} else return delegate.apply(window, args);
	});
	clearNative = patchMethod(window, cancelName, (delegate) => function(self, args) {
		...
		} else delegate.apply(window, args);
	});
}
function patchCustomElements(_global, api) {
	const { isBrowser, isMix } = api.getGlobalObjects();
	if (!isBrowser && !isMix || !_global["customElements"] || !("customElements" in _global)) return;
	api.patchCallbacks(api, _global.customElements, "customElements", "define", [
		"connectedCallback",
...
function patchBrowser(Zone) {
	...
	Zone.__load_patch("EventTarget", (global, Zone, api) => {
		...
		if (XMLHttpRequestEventTarget && XMLHttpRequestEventTarget.prototype) api.patchEventTarget(global, api, [XMLHttpRequestEventTarget.prototype]);
	});
	...
	Zone.__load_patch("XHR", (global, Zone) => {
		...
		function patchXHR(window) {
			...
			const sendNative = patchMethod(XMLHttpRequestPrototype, "send", () => function(self, args) {
				if (Zone.current[fetchTaskScheduling] === true) return sendNative.apply(self, args);
				if (self[XHR_SYNC]) return sendNative.apply(self, args);
					...
					if (self && self[XHR_ERROR_BEFORE_SCHEDULED] === true && !options.aborted && task.state === SCHEDULED) task.invoke();
				}
			});
			const abortNative = patchMethod(XMLHttpRequestPrototype, "abort", () => function(self, args) {
				...
				} else if (Zone.current[fetchTaskAborting] === true) return abortNative.apply(self, args);
			});
		}
	});
	Zone.__load_patch("geolocation", (global) => {
		if (global["navigator"] && global["navigator"].geolocation) patchPrototype(global["navigator"].geolocation, ["getCurrentPosition", "watchPosition"]);
	});
			...
			if (promise[symbolState] === UNRESOLVED) {
				...
				if (state !== REJECTED && value instanceof ZoneAwarePromise && value.hasOwnProperty(symbolState) && value.hasOwnProperty(symbolValue) && value[symbolState] !== UNRESOLVED) {
					...
					if (state === REJECTED && value instanceof Error) {
						...
						if (trace) ObjectDefineProperty(value, CURRENT_TASK_TRACE_SYMBOL, {
							configurable: true,
							enumerable: false,
							writable: true,
							value: trace
...
function patchUtil(Zone) {
	Zone.__load_patch("util", (global, Zone, api) => {
		...
		if (global[SYMBOL_BLACK_LISTED_EVENTS]) Zone[SYMBOL_BLACK_LISTED_EVENTS] = Zone[SYMBOL_UNPATCHED_EVENTS] = global[SYMBOL_BLACK_LISTED_EVENTS];
